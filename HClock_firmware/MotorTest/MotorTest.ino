...
void loop(void) {
  if (Serial.available() > 0) {          // Check receive buffer.
    ...
    if (rxString.equals("move")) {
      Serial.println("Performing continuous rotations...");
    ...
    else if (rxString.equals("rotate")) {
      Serial.println("Performing full rotation...");
      // turn on motor driver using MOSFET, while coil pins are still in the old state
      digitalWrite(MOSFET_PIN, HIGH);
      for (int ii = 0; ii < nROTATION; ii++)
      {
        Serial.print("Step ");
        Serial.println(ii+1);
        stepper.step(1);
    ...
    else if (rxString.equals("clock")) {
      Serial.println("simulate Clock behaviour");
      millisLast = millis();
      while(stepsPerformed < nROTATION)  {
        ESP.wdtFeed(); // reset watchdog timer
        if ( (millis() - millisLast)/2500 >= nMinutes+1 ) {
          ...
          {
            Serial.print("Step ");
            Serial.println(stepsPerformed + ii + 1);
            stepper.step(1);
            delay(100);
          }
          ...
          // output current time
          int cHour   = hourZero   + (minuteZero + nMinutes)/60;
          int cMinute = (minuteZero + nMinutes)%60;
    ...
    else if (rxString.equals("home")) {
      homingCycle();
    }

    else if (rxString.equals("toCurrent")) {
      Serial.print("Moving to current time: ");
      Serial.print(hour);
      Serial.print(":");
      Serial.println(minute);
      getTime();
      fromZeroToTime(hour, minute);
    }

    else {
      hour   = rxString.substring(0, 2).toInt() % 12;
      minute = rxString.substring(3, 5).toInt();
...
void homingCycle() {
  ...
  // keep track of steps to zero position
  int stepsToZero = ;
  // allow movement
  digitalWrite(MOSFET_PIN, HIGH);
  ...
  Serial.print("Steps to Zero position: ");
...
void fromZeroToTime(int h, int m) {
  ...
  minuteDiff = h * 60 + m - (hourZero * 60 + minuteZero);
